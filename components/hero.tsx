...
const Hero = ({ content, heroTitleVisible }: HeroProps) => {
  const heroImage = content?.home?.hero?.image || ''
  const heroTitle = content?.home?.hero?.title || ''
  const heroSubtitle = content?.home?.hero?.subtitle || ''
  const heroSubtitle2 = content?.home?.hero?.subtitle2 || ''
  ...
  return (
    <div 
     ...
     >
       {/* Hero Section */}
       <section className="relative pt-16 pb-16 px-8 md:px-8 min-h-[80vh] flex items-center">
